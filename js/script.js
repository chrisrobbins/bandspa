...
function loadBands() {
  $.ajax({
      url: 'https://bandlistapi.herokuapp.com/bands'
    }).done(function(data) {
      console.log(data);
...
function loadBand(band) {
  ...
  li.html(band.name + ' ');
   var a = $('<a>Delete</a>');      
   a.attr('href','http://bandlistapi.herokuapp.com/bands' + band._id);     a.addClass('deleteBand');    
   li.append(a);    
...
function loadNewBand() {
  $('#new_band_form').on('submit', function(e) {
    ...
    var genre = $('#genre').val()
    $.ajax({
      url: 'http://bandlistapi.herokuapp.com/bands',
      method: 'POST',
